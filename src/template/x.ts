...
export default async (van: VanObj) => {
  const {tags: {div, i, li, p, strong, ul}} = van
  const {ApiTable, Caveat, Demo, Download, H1, H2, H3, Html, InlineHtml, InlineJs, InlineTs, Js, Link, Shell, SymLink, Symbol, Ts, VanJS, VanX, jsFiddle} = common(van)

  const version = await fsp.readFile('assert/code/van-x.version', 'utf8')

  const codes = {
    name: await fsp.readFile('src/template/x/name.code.js', 'utf8'),
    nameCalc: await fsp.readFile('src/template/x/name-calc.code.js', 'utf8'),
    nameStateFields: await fsp.readFile('src/template/x/name-state-fields.code.js', 'utf8'),
    nameExistingClass: await fsp.readFile('src/template/x/name-existing-class.code.js', 'utf8'),
    todoApp: await fsp.readFile('src/template/demo/todo-app.code.js', 'utf8'),
    list1: await fsp.readFile('src/template/x/list1.code.js', 'utf8'),
    list2: await fsp.readFile('src/template/x/list2.code.js', 'utf8'),
  }
...
`),
  ...
  fullName: vanX.calc(() => \`\${data.name.first} \${data.name.last}\`),
    ...
    Js(codes.nameStateFields),
    p(Demo()),
    p({id: "demo-name-state-fields"}),
    jsFiddle({
      id: "name-state-fields",
      "data-details": "demo-van-x.details",
      "data-prefix": "const {button, div, input} = van.tags",
      "data-suffix": "van.add(document.body, Name())",
      "data-css": "input { width: 90px; }\n",
      code: codes.nameStateFields,
    }),
    ...
    Js(codes.nameExistingClass),
    p(Demo()),
    p({id: "demo-name-existing-class"}),
    jsFiddle({
      id: "name-existing-class",
      "data-details": "demo-van-x.details",
      "data-prefix": "const {button, div, input} = van.tags",
      "data-suffix": "van.add(document.body, Name())",
      "data-css": "input { width: 90px; }\n",
      code: codes.nameExistingClass,
    }),
