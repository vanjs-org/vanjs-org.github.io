...
`),
    ...
    p("We might be prompted to assign a DOM node to a ", Symbol("State"), " object, especially when the ", Symbol("State"), " object is used as a ", Symbol("State"), "-typed child. However, this is problematic when the state is bound to multiple DOM nodes, like the example below:"),
    JsFile("dom-valued-state.code.js"),
    p(Demo(), span({id: "demo-dom-valued-state"})),
    p({id: "jsfiddle-dom-valued-state"}),
    p("In this example, if we click the button \"Turn Bold\", the first \"", VanJS(), "\" text will disappear, which is unexpected. This is because the same DOM node is used twice in the DOM tree. For this reason, an error will be thrown in ", Symbol("van-{version}.debug.js"), " whenever we assign a DOM node to a ", Symbol("State"), " object."),
