...
`)),
    ...
    p("The solution to the problem is to capitalize some letters in the properties of the tag function. For instance, a good option is to use ", Symbol("onClick"), " property instead of ", Symbol("onclick"), ". This way, ", InlineJs(`button({onClick: "play()"}, "Play")`), " will generate ", InlineHtml(`<button onclick="play()">Play</button>`), " in both ", VanJS(), " and ", MiniVan(), ". This is because ", Symbol("onClick"), " is not a valid DOM property, thus in tag functions, we will set the specified corresponding string value as HTML attributes (You can refer to the ", Link("property resolution rule", "/advanced#dom-attributes-vs-properties"), " for more information). And also ", InlineJs(`dom.setAttribute(<key>, <value>)`), " will first lowercase the ", Symbol("<key>"), " before setting the attribute value, which means ", InlineJs(`dom.setAttribute("onClick", "play()")`), " is equivalent to ", InlineJs(`dom.setAttribute("onclick", "play()")`), "."),
  )
}
